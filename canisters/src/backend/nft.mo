...
import Types "./nft_types";

actor class StoryNFT(
  custodian : Principal,
  ...
  public query func balanceOfDip721(user : Principal) : async Nat64 {
    return Nat64.fromNat(
      List.size(
        List.filter(
          nfts,
          func(token : Types.Nft) : Bool { token.owner == user },
        ),
      ),
    );
  };
  ...
  public query func ownerOfDip721(token_id : Types.TokenId) : async Types.OwnerResult {
    let item = List.find(
      nfts,
      func(token : Types.Nft) : Bool { token.id == token_id },
    );
    switch (item) {
      case (null) {
  ...
  func transferFrom(
    ...
    switch (item) {
      ...
      case (?token) {
        if (
          caller != token.owner and not List.some(
            custodians,
  ...
  public func getMetadataForUserDip721(user : Principal) : async Types.ExtendedMetadataResult {
    ...
    switch (item) {
      ...
      case (?token) {
        return #Ok({ metadata_desc = token.metadata; token_id = token.id });
      };
    };
  };
  ...
  public query func getTokenIdsForUserDip721(user : Principal) : async [Types.TokenId] {
    ...
    let tokenIds = List.map(
      items,
      func(item : Types.Nft) : Types.TokenId { item.id },
    );
  ...
  public shared ({ caller }) func mintDip721(
    ...
    let newId = Nat64.fromNat(List.size(nfts)) + 1;
    let nft : Types.Nft = {
      owner = to;
      id = newId;
      metadata = metadata;
    ...
    return #Ok({ token_id = newId; id = transactionId });
  };
};
