...
export async function readV1(script: ScriptV1): Promise<ScriptReader> {
  const zh = await loadJson<Record<string, string>>(script.locales.zh);
  const en = await loadJson<Record<string, string>>(script.locales.en);

  let outline = null;
  if (script.mode === Script.Mode.Game) {
    outline = await loadJson<{
      "zh-CN": Script.Outline;
      "en-US": Script.Outline;
    }>(`res/story-outline/${script.name}-${script.name}.json`);
  }
  const openLetter = await loadJson<{
    "zh-CN": Script.OpenLetter;
    "en-US": Script.OpenLetter;
  }>(`res/open-letter/${script.name}-${script.mode}.json`);
